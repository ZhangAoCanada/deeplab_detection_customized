...
class DeepLabModel(object):
  ...
  def run(self, image):
    ...
    return resize_ratio, resized_image, batch_seg_map
...
def vis_segmentation(image, seg_map):
  ...
  grid_spec = gridspec.GridSpec(1, 2, width_ratios=[5, 1])
...
def runImg(MODEL, img_path):
    original_img = Image.open(img_path)
    if original_img is None:
        raise ValueError("Image does not exist")
    print('running deeplab on image %s...' %(img_path))
    resize_ratio, resized_im, seg_maps = MODEL.run(original_img)
    seg_map = seg_maps[0]
...
def main(img_path):
    ...
    print("--------------------------------------------")
    runImg(MODEL, img_path)

if __name__ == "__main__":
    img_path = "/Users/aozhang/Documents/test_img.jpeg"
    main(img_path)
